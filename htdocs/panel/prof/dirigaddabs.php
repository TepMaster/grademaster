...
<script type="application/javascript">
    function run(id){
        ...
        swal({
            title: "Sunteti sigur?",
            text: "Odata ce ati sters absenta acesta nu va putea fi recuperata!",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swal("Success!", "Poof! Absenta a fost stersa!!", "success").then(function() {
                ...
                <div class="rep-cards" style="">
                        ...
                        <div class="comp-tabs">
                            <div class="scroll-div">
                                <button type="button" class="page-tabs__button _tab" data-count="0" id="item-0">
                                    <a aria-current="page" href="/dirigmanage?student=<?php echo $_GET["student"];?>&materie=<?php echo $_GET["materie"];?>">Note</a>
                                </button>
                                <button type="button" class="page-tabs__button _tab" data-count="1" id="item-1">
                                    <a href="/addaps?student=<?php echo $_GET["student"];?>&materie=<?php echo $_GET["materie"];?>" class="active">Absențe</a>
                                </button>
                    ...
                    <div class="report-card-grades-container false">
                        <table>
                            ...
                            <?php
                            $stat = DB::query("SELECT * FROM absente WHERE userid =%s and moterieid =%s ",$_GET["student"],$_GET["materie"]);
                            for($i=0;count($stat)>$i;$i++){
                                ...
                                $mes ="";
                                if($stat[$i]["mot"]==0){$mes="NU";}else{$mes="DA";}
                                if($_SESSION["rank"]==1){
                                    echo '<tr>
                               <td>'.$stat[$i]["data"].'</td>
                                 <td>'.$obs["n"].'</td>
                                 <td>'.$mes.'</td>
                                 <td><a href="/motiveaza?id='.$stat[$i]["id"].'"><button class="sbutton">Motiveaza</button></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
                               ...
                               <td>'.$stat[$i]["data"].'</td>
                                <td>'.$obs["n"].'</td>
                                 <td>'.$mes.'</td>
                                 <td><a href="#"><button class="sbutton">Motiveaza</button></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<button class="sbutton" onclick="run('.$stat[$i]["id"].')">Sterge Absenta</button></td>
                        ...
                        <br>
                        <a href="/addabs?student=<?php echo $_GET["student"];?>&materie=<?php echo $_GET["materie"];?>"><button style="background-color: #008CBA; position: absolute;left: 80%;padding: 20px 35px;font-size: 16px;" class="sbutton">Adauga Absenta</button></a>
                    </div>
                </div>
            </div>
        </div>
