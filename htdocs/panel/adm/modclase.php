...
$materi="";
if(isset($_GET['id'])){
    $row = DB::query("SELECT * FROM clase WHERE id =%s",$_GET["id"]);
   $nume = $row[0]["nume"];$dirig = $row[0]["dirig"];
   $mat = unserialize($row[0]["materi"]);
   $frst =1;
   foreach ($mat as $lol){
       if($frst==1){$materi .= $lol; $frst=0;}else{
           $materi .=','.$lol;;
       }
            ...
            <div style="box-sizing: border-box; min-height: 1px; position: relative; padding-left: 0px; padding-right: 0px; width: 83.3333%; flex: 0 0 83.3333%; max-width: 83.3333%; margin-left: 0%; right: auto; left: auto;">
                <div class="rep-cards" style="">
                    ...
                    <div class="form">
                        ...
                        <h2>Modificare Clasa</h2>
                        <form method="post" enctype="multipart/form-data" action="#">
                            <label for="grade">Nume Clasa:</label>
                            <input type="text" id="num" name="num" min="0" max="100" required value="<?php echo $nume?>">
                            <br><br>
                            <label for="grade">Nume Diriginte:</label>
                            <input type="text" id="pass" name="pass" min="0" max="60" required value="<?php echo $dirig?>">
                            <br><br>
                            <label for="grade">Materi:</label>
                            <input type="text" id="em" name="em" min="0" max="60" required value="<?php echo $materi?>">
                            <br><br>&nbsp;
                            <a href="/adm/materi">Link materi</a>
                            <br>&nbsp;<br><br>
...
if(isset($_POST["num"])){
    $str_arr = explode (",", $_POST["em"]);

    DB::update('clase', [
        'materi' =>  serialize($str_arr),
        'dirig' => $_POST["pass"],
        'nume' => $_POST["num"]
    ], "id=%s", $_GET['id']);
    ...
    'swal("Clasa Modificata!", "Ati modificat clasa cu succes!", "success").then(function() {
